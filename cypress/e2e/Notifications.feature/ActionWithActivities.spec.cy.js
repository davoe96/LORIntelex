describe ('As a system with Notifictions configured I want to send out an email. So that when an Action Plan with Activities is created and assigned, the assignee receives the expected notification to action', () =>{
    describe ('Action Plan with Activities Assignment Notification',() =>{
    beforeEach(() => {
     //cy.clearCache();
    ...
    it(' Action Plan assignee Filter notifications by text and timestamp', function() {
        ...
        cy.get('#sysNotificationInventory_Filter_MessageBase_Template_Value')
            .type('Action Plan with Activities assigned to you{enter}', { force: true });
            ...
            cy.get('table tbody tr.data_row').each(($row) => {
                ...
                if (Recipient.includes('David Aboim') && Template.includes('Action Plan with Activities assigned to you.')) {
